...
void CWallet::adChanged(const CTransaction& tx, const CBlock* const pblock,
    const bool fConnectOrDisconnect)
{
  // from main.cpp  fUpdate     fConnect
  //CT_NEW          true        true
  //CT_UPDATED
  //CT_DELETED      false       false
  NotifyAdChanged(tx.GetHash(), fConnectOrDisconnect ? CT_NEW : CT_DELETED);
}
